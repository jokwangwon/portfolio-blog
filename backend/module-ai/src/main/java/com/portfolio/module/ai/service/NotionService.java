...
public class NotionService {
    ...
    public NotionService(
            ...
            @Qualifier("notionPageCache") Cache<String, String> notionPageCache) {
    ...
    /**
     * Notion 페이지를 읽어 마크다운 형태로 반환한다.
     * Caffeine 캐시를 통해 동일 페이지 반복 조회 시 캐시된 결과를 반환한다.
     */
    public Mono<String> readPage(String pageId) {
        ...
        return Mono.zip(titleMono, contentMono)
                ...
                .onErrorMap(WebClientResponseException.NotFound.class,
                        e -> new ResourceNotFoundException("NOTION_PAGE_NOT_FOUND",
                                "Notion 페이지를 찾을 수 없습니다: " + pageId))
                .onErrorMap(WebClientResponseException.Unauthorized.class,
                        e -> new ResourceNotFoundException("NOTION_UNAUTHORIZED",
    ...
    /**
     * 페이지의 모든 블록을 재귀적으로 가져온다 (Parallel BFS).
     * cursor-based pagination을 사용하며, 자식 블록이 있으면 병렬로 가져온다.
     */
    ...
    /**
     * cursor-based pagination으로 블록 목록을 가져온다.
     * expand()를 사용하여 has_more가 true인 동안 반복 요청한다.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * 블록 목록을 재귀적으로 처리한다.
     * 자식이 있는 블록은 MAX_CONCURRENCY로 병렬 처리한다.
     */
    ...
    record BlockPageResult(List<Map<String, Object>> results, boolean hasMore, String nextCursor) {}
}
