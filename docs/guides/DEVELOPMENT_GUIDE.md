...
### 11.1 Phase 1 한정: 문서 기반 SSOT

Phase 1(1인 Vibe Coding 체제)에서는 **`docs/sessions/`를 단일 진실 공급원(SSOT)**으로 사용합니다.

| 항목 | 도구 | 비고 |
|------|------|------|
| 작업 추적 | `docs/sessions/SESSION_*.md` | 세션별 작업 기록 |
| 현재 상태 | `docs/CONTEXT.md` | 최신 프로젝트 상태 |
| 의사결정 | `docs/decisions/ADR-*.md` | 아키텍처 결정 기록 |
| 이슈 관리 | GitHub Issues (보조) | 외부 공유/검색용 |

> **왜 GitHub Issues 대신 문서인가?**
> AI 에이전트가 `.md` 파일을 직접 읽고 컨텍스트로 주입할 수 있어, 티켓 시스템보다 작업 연속성이 높습니다.

> **Phase 2 전환 시**: GitHub Issues/Projects를 주 추적 도구로 전환하고, 세션 로그는 보조 아카이브로 격하합니다.

### 11.2 PR과 멀티 에이전트 합의

아키텍처, SDD, 보안에 관련된 **큰 결정**이 포함된 PR에는 반드시 3+1 멀티 에이전트 합의 리포트를 첨부합니다 (ADR-007 참조).

- PR 템플릿의 "멀티 에이전트 합의" 체크박스를 확인
- 합의 리포트는 PR 본문 또는 별도 파일로 첨부
- 일반 코딩/버그 수정은 합의 불필요
...
- [ ] 문서를 업데이트했는가?
- [ ] 커밋 메시지가 규칙을 따르는가?
- [ ] 아키텍처/SDD/보안 변경 시 멀티 에이전트 합의 리포트를 첨부했는가? (ADR-007)
